...
COLORS = { 'is': '#000000',
           'b-': '#62aeff',
           '2b-': '#62aeff',
           'b+': color_to_grayscale_hex('#ff7e75'),
           'ec': color_to_grayscale_hex('#ff7e75'),
           '2ec': '#ff7e75',
           'a' : '#fffe49',
           'sf': '#5cbc57',
           'p' : '#ffa425',
...
# JEM addition:
def _draw_unfilled_rectangle(layer, position, stroke_color, name):
    """Draws rectangle (basic nuclide on map) position is
    given for left top corner """
    x = position[0]
    y = position[1]
    ...
    rectangle.setAttribute("width", str(SIZE_SHAPE))
    rectangle.setAttribute("height", str(SIZE_SHAPE))
    rectangle.setAttribute("stroke", stroke_color)
    rectangle.setAttribute("stroke-width", "4.0")
    # rectangle.setAttribute("fill", color)
    rectangle.setAttribute("x", str(position[0]))
    rectangle.setAttribute("y", str(position[1]))
    layer.appendChild(rectangle)
...
def draw_nuclide(nuclide, layers, position, args):
    ...
    # JEM HACK FOR CECILIE
    if nuclide.comment == "target":
        # Drawing unfilled rect in layer 4, which I added
        position_target = position
        position_compound = [position[0]
                             + 2 * SIZE_FIELD,
                             position[1] - 1 * SIZE_FIELD
                             ]
        _draw_unfilled_rectangle(layers[4], position_target,
                                 "#C70AC7", '{}0'.format(nuclide))
        # Drawing unfilled rect in layer 4, which I added
        _draw_unfilled_rectangle(layers[4], position_compound,
                                 "#601DC7", '{}0'.format(nuclide))
        # _draw_line(layers[4],
        #            [position_target[i] + 0.5*SIZE_FIELD for i in range(len(position_target))],
        #            [position_compound[i] + 0.5*SIZE_FIELD for i in range(len(position_target))],
        #            "")
...
if __name__ == "__main__":
    ...
    # JEM hack for Cecilie:
    # Use me with the following command:
    # ./ChartDrawer-Cecilie-allregions.py --z 47 52 --n 64 78 --halflives nubase16.xml chart-Cecilie-region1.svg


    target_list = [(48, 68),  # 116Cd
                   (50, 74)  # 124Sn
                   ]
    compound_list = [(49, 70),  # 116Cd(alpha, p)
                   (51, 76)  # 124Sn(alpha, p)
                   ]
    ...
    for nuclide in data:
        ...
        if (nuclide.Z, nuclide.N) in target_list:
            nuclide.comment = "target"
            print("coloring target nucleus Z =", nuclide.Z, "N =", nuclide.N)
        if (nuclide.Z, nuclide.N) in compound_list:
            nuclide.comment = "compound"
            print("coloring compound nucleus Z =", nuclide.Z, "N =", nuclide.N)
